...
Calculatestes marketing channel contribution to conversions using multiple attribution models (first-touch, last-touch, linear, time-decay, position-based, and data-driven Shapley value) to determine accurate channel ROI and optimize marketing spend allocation.
...
|-----------|------|----------|---------|-------------|------------------|
| journey_data | array | Yes | - | array of customer journey objects containing touchpoint sequences and conversion outcomes. | Complete customer interaction history across all marketing channels. |
| attribution_models | array | No | `["last_touch", "first_touch", "linear", "time_decay", "shapley"]` | Attribution models to apply. Available: `first_touch`, `last_touch`, `linear`, `time_decay`, `position_based`, `shapley`. | Different methodologies for crediting channels. Use multiple models to compare and validate insights. |
...
| conversion_value_field | string | No | `"revenue"` | Field name containing conversion value (for revenue-weighted attribution). | Which metric to use for attribution: revenue, profit, lifetime_value, etc. |
...
|-------|------|----------|-------------|---------|
| journey_id | string | Yes | Unique identifier for the customer journey. | `"J12345"` |
| touchpoints | array | Yes | Ordered array of marketing touchpoint interactions. | `[{"channel": "paid_search", "timestamp": "2025-11-01T10:00:00Z"}, ...]` |
| converted | boolean | Yes | Whether the journey resulted in conversion. | `true` |
| conversion_value | float | Conditional | Value of the conversion (Obrigatório if using value-weighted attribution). | `299.99` |
...
| Field | Type | Required | Description | Example |
|-------|------|----------|-------------|---------|
| channel | string | Yes | Marketing channel name (e.g., paid_search, email, organic, social). | `"paid_search"` |
| timestamp | string | Yes | ISO 8601 timestamp of the touchpoint interaction. | `"2025-11-01T10:00:00Z"` |
...
```json
{
  ...
  "details": {
    ...
    "journey_data.0.converted": "The converted field is required and must be to boolean."
  }
}
```
...
| `attribution_results.{model}.{channel}` | object | Channel attribution metrics for to specific model. | How much credit to channel receives under to particular attribution methodology. |
...
| `model_comparison.comparison_matrix` | array | Matrix where rows are channels and columns are models, values are percentage attribution. | Quick visual comparison of model differences. Identify channels with consistent vs. variable attribution. |
...
| `summary` | object | Aggregate statistics across all journeys and models. | Overall campaign performance and data quality indicators. |
...
| `summary.total_journeys_analyzed` | integer | Number of customer journeys analyzed (converted and non-converted). | Sample size for statistical confidence. |
...
### 1. Data Collection
Extract customer journey and touchpoint data from marketing platforms:
...
- Balance short-term (last-touch) and long-term (first-touch) value

### 7. Monitor and Iterate
Track attribution over time:
- Re-run monthly to detect shifts in channel effectiveness
- Compare attribution before and after marketing changes
- Use `group_by_period` to analyze seasonal trends

## Perguntas Frequentes

### Q: Which attribution model should I use?
A: Start with Shapley value for multi-touch journeys as it provides the most statistically rigorous attribution based on game theory. Use first-touch to underste awareness drivers, last-touch for immediate conversion drivers, and time-decay for recency-weighted attribution. Running multiple models simultaneously helps validate insights.

### Q: What does it mean if different models show very different results for to channel?
A: High variance across models indicates that the channel's role varies significantly depending on its position in the journey. For example, to channel showing 40% attribution in first-touch but only 10% in last-touch is primarily an awareness driver, not to converter. Use this to align channel strategy with its actual role.

### Q: How of the I interpret Shapley values?
...
### Q: What is to good ROAS (Return on Ad Spend)?
A: Industry benchmarks vary widely. E-commerce typically aims for 400-600% ROAS (4-6x return), B2B SaaS for 300-500%, and bre awareness for 200-300%. Use your own historical data as to baseline and optimize from there. ROAS below 100% means you're spending more than you're earning.

### Q: Should I only look at last-touch attribution?
A: Não. Last-touch significantly undervalues awareness and nurturing channels. A customer might discover your bre through paid social, research via organic search, and convert via email. Last-touch gives 100% credit to email, ignoring the critical role of paid social and organic. Multi-touch attribution reveals the full customer journey.

### Q: How many journeys of the I need for reliable attribution?
...
| **Email Undervalued** | If email shows <10% last-touch attribution but 25%+ in linear or Shapley, it's to critical nurturing channel. Increase email cadence and personalization. | Improve conversion rates by 20-30% through better email nurturing. |
| **Paid Social Low ROI** | If paid social ROAS <200% and Shapley attribution <15%, reduce spend or shift to retargeting only. Test creative and audience targeting before cutting entirely. | Reduce wasted ad spend by 30-40%, reallocate to higher-ROI channels. |
...
| **First-Touch Dominance** | If first-touch and last-touch agree on top channels, your funnel is efficient. Optimize conversion rates on these proven channels rather than expeing channel mix. | Increase conversion rate by 15-25% through channel-specific optimization. |
| **Attribution Volatility** | If model_agreement_score <0.50, your journeys are highly variable. Segment customers by journey Tipo and create channel strategies for each segment. | Improve marketing efficiency by 20-30% through segment-specific tactics. |
...
### 3. Desempenho and Optimization

- **Tempo of Processamento:** 150-300ms for 10,000 journey paths
- **Requisitos of Dados:** Minimum 100 complete customer journeys for reliable attribution
...
- **Time-Decay** provides to good balance between accuracy and performance for most use cases
...
- **Use when:** First and last touches are most valuable (U-shaped funnel)
- **Best for:** Traditional marketing funnels with clear awareness and conversion stages
...
- Remove bot traffic and invalid interactions

**Journey Completeness:**
- Include all touchpoints, not just paid channels
- Track organic, direct, and referral traffic
- Capture both online and offline touchpoints when possible
...
- **Tempo of Processamento:** 150-300ms for 10,000 journeys, 1-3 seconds for Shapley with 100,000 journeys
...
**Attribution Window:**
- Default: 30-day attribution window (touchpoints within 30 dias of conversion)
...
- Cost data is aggregated and anonymized in reports
