...
Extracts engineered features from customer data for Customer Lifetime Value (CLV) prediction modeling. This Endpoint prepares behavioral, transactional, and demographic features that serve as inputs for CLV forecasting algorithms.
...
| --------- | ---- | -------- | ----------- | ---------------- | ------- |
| customer_data | array | Yes | array of customer registros with transaction history. | Customer profiles with purchase behavior data. | - |
...
| include_metadata | boolean | No | Include feature engineering metadata and descriptions. | Adds explanations for each computed feature. | `false` |

### Customer Data object Fields

| Field | Type | Required | Description | Significado Empresarial | Example |
| ----- | ---- | -------- | ----------- | ---------------- | ------- |
| customer_id | string | Yes | Unique customer identifier. | Customer account ID or email. | `"CUST-10234"` |
| transactions | array | Yes | array of transaction registros. | Purchase history with dates and amounts. | See below |
...
| ----- | ---- | -------- | ----------- | ------- |
| transaction_id | string | Yes | Unique transaction identifier. | `"TXN-789012"` |
| date | string | Yes | Transaction date (ISO 8601). | `"2025-10-15"` |
| amount | float | Yes | Transaction amount in currency units. | `149.99` |
...
| `features[].temporal_features` | object | Time-based behavior patterns. | Trends and velocity metrics. |
...
| `metadata` | object | Feature engineering metadata. | Processing information and versioning. |
...
| `metadata.computation_time_ms` | integer | Duração of the processamento em milisseconds. | Métrica of desempenho. |
...
2. **Engenharia of Características:** Calculate derived features (e.g., 30/60/90-day purchase trends)
...
- **Tempo of Processamento:** 150-400ms for 5,000 customers with full feature extraction
- **Requisitos of Dados:** Minimum 3 meses of customer transaction history
- **Tamanho of the Lote:** Process up to 50,000 customers per request
...
- **Qualidade of Dados:** Ensure transaction dates are accurate and amounts are positive
...
3. Use predictions for customer segmentation and targeting
...
**A:** Demographic fields (age, location) are optional and of the not affect core feature extraction. If provided, they're passed through in the Resposta for downstream model usage. Missing demographics don't impact transactional, temporal, or behavioral feature computation.

### Q: Can I use these features directly for CLV prediction?
**A:** Sim! These features are designed as inputs for CLV forecasting models. Use the `all` feature set for comprehensive modeling. The features are pre-normalized and ready for machine learning algorithms. For automated CLV prediction, use the [CLV Forecast](CustomerClvForecast.md) Endpoint, which internally uses these features.

### Q: What does the trend_coefficient represent and how is it calculated?
...
**A:** For real-time applications (e.g., personalization engines), refresh features weekly or when significant customer actions occur (new purchase, large order). For monthly reporting and batch campaigns, refresh mensal. Features are stateless—each API call computes fresh features from provided transaction data without caching.

### Q: Can I process customers with different currencies?
**A:** Sim, but ensure all amounts for to given customer are in the same currency. The system doesn't perform currency conversion—it treats all amounts as numeric values. For multi-currency customers, pre-convert transactions to to single base currency (e.g., USD) before submission. Currency metadata isn't stored in features.
...
2. Identify customers with low diversity (<3 categories) and high frequency (5+ purchases)
...
**Resultados Esperados:**
- 15-20% of dormant customers make to purchase
...
4. Monitor conversion rates and AOV by segment
