...
| data[].total | object  | Totalizadores da plataforma (condicional - apenas se houver usuários) |
| data[].total.users | integer | Total de usuários na plataforma |
| data[].is_parent | boolean | Indica se a plataforma é uma plataforma pai |
| data[].domain | object  | Informações da área de domínio |
| data[].domain.uuid | string  | Identificador único da área de domínio |
| data[].domain.slug | string  | Slug da área de domínio |
...
Os campos abaixo são agrupados dentro do objeto `company` e só aparecem quando a plataforma possui uma empresa associada:

| Campo | Tipo | Descrição |
| ----------- | ------- | ----------- |
| data[].company | object  | Dados da empresa (condicional) |
| data[].company.uuid | string  | Identificador único da empresa |
| data[].company.name | string  | Nome da empresa |
| data[].company.slug | string  | Slug da empresa |
| data[].company.zipcode | string\|null  | CEP da empresa |
| data[].company.country | string\|null  | País da empresa |
| data[].company.state | string\|null  | Estado da empresa |
| data[].company.city | string\|null  | Cidade da empresa |
| data[].company.company_logos | array   | Logos da empresa (array vazio se não carregado) |
| data[].company.full_address | string\|null  | Endereço completo formatado |
| data[].company.links | array   | Links de redes sociais da empresa (array vazio se não carregado) |
| data[].company.links[].uuid | string  | Identificador único do link |
| data[].company.links[].platform | string  | Nome da plataforma social |
| data[].company.links[].url | string  | URL do perfil social |

### Metadados de Paginação

| Campo | Tipo | Descrição |
| ----------- | ------- | ----------- |
| links       | object  | Links de navegação entre páginas |
...
| meta.from | integer | Número do primeiro registro da página |
| meta.last_page | integer | Número da última página |
| meta.per_page | integer | Registros por página |
| meta.to | integer | Número do último registro da página |
...
- `domainArea` - Área de domínio da plataforma
- `company` - Empresa associada à plataforma
- `company.logos` - Logos da empresa
- `company.address` - Endereço da empresa (usado para country, state, city)
- `company.socialMedias` - Redes sociais da empresa

## Contadores

- `users_count` - Total de usuários cadastrados na plataforma
...
- Apenas usuários com permissão `index.all` podem acessar
- Ordenado por data de criação (decrescente) - as mais recentes aparecem primeiro
- O campo `total.users` só aparece quando a plataforma possui usuários cadastrados
- O objeto `company` só aparece quando a plataforma possui uma empresa associada
- Os arrays `company_logos` e `links` retornam array vazio quando as relações não estão carregadas
- Todas as URLs de logo são completas e prontas para uso
- As datas seguem o formato ISO 8601
- A paginação padrão retorna 25 registros por página
- Use `no_paginate=true` para obter todos os resultados sem paginação
...
- 2025-10-25: Atualização completa da documentação com nova estrutura de company e campos adicionais
