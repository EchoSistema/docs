...
Performs forecasting of future revenue using time series analysis e machine learning models. The system employs a model zoo approach, testing multiple algorithms to select the champion model with the best performance.
...
| Parâmetro          | Tipo   | Obrigatório | Descrição | Padrão | Constraints |
| ------------------ | ------ | ----------- | --------- | ------- | ----------- |
| historical_revenue | array  | Sim         | Historical revenue data. Each element must contain `date` e `revenue` fields. | - | Minimum 60 data points (approximately 2 meses of daily data) |
| forecast_periods   | int    | Sim         | Number of future periods to forecast. | - | Range: 1-365 dias |
| granularity        | string | Não         | Temporal granularity for forecasting. | `monthly` | Values: `daily`, `weekly`, `monthly` |
| confidence_level   | float  | Não         | Confidence level for prediction intervals. | `0.95` | Range: 0.80-0.99 |
| external_factors   | object | Não         | External factors affecting revenue (seasonality, promotions, holidias, etc.). | `null` | Optional object with custom factors |
...
The API returns forecasts in two distinct views: calendar view (all dias) e business dias view (working dias only).
...
| --------------------------------------- | ------- | --------- |
| `forecast_calendar`                     | array   | Forecast for all calendar dias (including weekends e holidias). |
...
| ----------- | ----------- |
| `200 OK` | Forecast generated successfully. Resposta includes dual-view forecasts e model metrics. |
...
| `429 Too Many Requests` | Limite de taxa excedido. Retry after the specified time. |
...
| Invalid confidence level | `400` | Confidence level outside range 0.80-0.99. | Provide confidence level between 0.80 e 0.99. |
...
### Erro Example: Insufficient Data
...
### Erro Example: Invalid Date Format
...
- **Use case:** Time series with trends e autocorrelation
...
- **Use case:** Time series with strong seasonality e holidias
...
The champion model is selected based on the **lowest weighted RMSE** across both calendar e business day views:
...
1. **Outlier Detection via IQR (Interquartile Range)**
   - Identifies values outside Q1 - 1.5×IQR e Q3 + 1.5×IQR
...
4. **Trend e Seasonality Decomposition**
   - Separates time series into trend, seasonal, e residual components
   - Uses STL decomposition (Seasonal e Trend decomposition using Loess)
...
- **Validation:** Each fold trains on historical data e validates on the next period
- **Metrics:** RMSE e MAE averaged across all folds
...
- **Minimum Requisitos de Dados:** At least 60 data points (approximately 2 meses of daily data) Obrigatório for reliable forecasting.
...
- **Granularity Impact:** Daily granularity requires more data points e may have higher variance than monthly granularity.

## Perguntas Frequentes

### Q: What's the minimum historical data period Obrigatório for reliable forecasts?
**A:** A minimum of 60 data points is Obrigatório (approximately 2 meses of daily revenue data). For seasonal products, we recommend 12+ meses of historical data to capture full seasonal cycles. Optimal data spans 24 meses for capturing trend e multiple seasonal patterns. For daily granularity, 180+ dias (6 meses) ensures accuracy. If you have less than 60 dias, upgrade to weekly or monthly granularity to reduce noise e improve predictions.

### Q: How does the model hele seasonality (holidias, peak seasons, etc.)?
**A:** The system automatically detects seasonality via STL decomposition e selects appropriate algorithms (Prophet e TBATS excel at seasonal patterns). The Resposta includes `decomposition` info showing if additive or multiplicative seasonality was detected. For strong seasonal patterns, Prophet typically ranks highest. We detect daily, weekly, monthly, e annual seasonality patterns. If you have custom seasonality (e.g., flash sales), include them in the optional `external_factors` Parâmetro for improved accuracy.
...
### Q: What's the difference between calendar e business dias forecasts?
**A:** Calendar view (`forecast_calendar`) includes all dias: weekends, holidias, e working dias. Use this for financial planning, budgeting, e reporting (total revenue projection). Business dias view (`forecast_business_dias`) excludes weekends e holidias, showing only operational dias. Use this for capacity planning, resource allocation, e operational decisions. Both use the same underlying model but provide different perspectives on the same forecast.

### Q: How do I know which model was selected e why?
**A:** The Resposta includes `champion_model` Campo showing the selected algorithm (e.g., "Prophet", "ARIMA", "ETS"). The `models_tested` array lists all 6 models with their weighted RMSE scores e rankings. The champion is the one with rank=1 (lowest weighted_rmse). Check the ranking to see if the champion significantly outperformed alternatives. Weighted RMSE combines calendar (60% weight) e business dias (40% weight) performance. A significant gap between 1st e 2nd place indicates high confidence in the champion.

### Q: Can I forecast daily revenue or only monthly?
**A:** You can forecast daily, weekly, or monthly via the `granularity` Parâmetro. Daily requires the most data (180+ dias recommended) e may have higher variance. Weekly is good balance. Monthly is most stable e requires less data. The API heles all granularities equally. Choose based on your business need e data availability. Daily is ideal for campaign management; monthly for executive reporting e budgeting. The Resposta structure is identical across granularities.

### Q: How do I interpret the confidence intervals (lower_bound e upper_bound)?
**A:** Confidence intervals represent the model's uncertainty range around the point prediction. Padrão 95% confidence means there's a 95% probability the actual value falls within [lower_bound, upper_bound]. Narrower intervals = higher confidence; wider intervals = more uncertainty. As forecast horizons extend (30, 60, 90+ dias out), intervals naturally widen. Use lower_bound for conservative planning (worst-case scenario) e upper_bound for optimistic planning. Adjust `confidence_level` (0.80-0.99) to balance tightness vs. reliability.

### Q: What do the RMSE, MAE, MAPE, e R-squared metrics mean?
...
### Q: How do I hele data quality issues like missing revenue values?
**A:** The system automatically imputes missing values: forward fill for gaps 1-2 periods, linear interpolation for 3-7 periods, e seasonal average for longer gaps. If > 80% of values are missing, the request fails with a clear Erro. Outliers are automatically detected via IQR e capped or removed. If you have systematic data quality issues (30%+ missing), clean the data before sending. The Resposta includes `preprocessing_applied` detailing all transformations so you underste what was corrected.
...
Specify the number of periods to forecast e granularity:
...
Review both calendar e business day forecasts:
- Use `forecast_calendar` for budgeting e financial planning
- Use `forecast_business_dias` for operational capacity planning

### Step 5: Evaluate Model Desempenho

Check the champion model e metrics:
