...
**Business Value:** Automatically classifies customer feedback sentiment with 88-92% accuracy, enabling rapid Resposta to negative experiences e identification of satisfaction drivers. Organizations typically see 20-30% faster issue resolution e 15% improvement in customer satisfaction scores.
...
|-----------|------|----------|---------|-------------|------------------|
| texts | array | Sim | - | array of text objects for sentiment analysis. Each object contains text content, ID, e optional metadata. | Customer feedback, reviews, comments, or survey responses to analyze for sentiment. |
| language | string | Não | `auto` | Text language Código (`en`, `es`, `pt`, or `auto` for automatic detection). | Improves accuracy when specified, but auto-detection works for 100+ languages. |
| include_entities | boolean | Não | `false` | Extract named entities (products, services, bres) mentioned in text. | Identifies what specific items customers are discussing in feedback. |
| include_aspects | boolean | Não | `false` | Perform aspect-based sentiment analysis to identify sentiment per feature/topic. | Shows sentiment for specific product features (e.g., "shipping", "quality", "support"). |
| sentiment_threshold | float | Não | `0.1` | Neutral zone threshold for classification (0-0.5). Higher values expe neutral category. | Adjusts sensitivity: lower threshold = more definitive classifications. |

### Text object Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
|-------|------|----------|-------------|---------|
| id | string | Sim | Unique identifier for the text (for tracking results). | `"review_001"`, `"comment_12345"` |
| text | string | Sim | Text content to analyze (max 10,000 characters per text). | `"Great product! Fast shipping e excellent customer service."` |
| source | string | Não | Source of the text (for categorization e reporting). | `"product_review"`, `"customer_feedback"`, `"social_media"` |
| metadata | object | Não | Additional metadata about the text (custom fields). | `{"product_id": "PROD-123", "customer_id": "C001"}` |
...
| `summary` | object | Aggregate statistics across all analyzed texts. | Overall sentiment distribution e averages for the batch. |
...
The sentiment analysis system uses state-of-the-art NLP models to classify text sentiment e extract insights:
...
- **Entity Recognition:** Uses Named Entity Recognition (NER) to identify products, services, e bres
...
### 1. Data Collection e Preparation
...
**Identify Patterns:**
- Group by sentiment category e analyze common aspects
...
- Track issue resolution e measure sentiment improvement

**For Positive Sentiment:**
- Identify bre advocates for testimonials e case studies
...
- Visualize sentiment trends e distribution
...
- Identify most-praised e most-criticized features
...
| **Competitive Intelligence** | Analyze competitor product reviews e social mentions using sentiment e entity extraction. Compare sentiment scores for similar features across bres. Identify competitive weaknesses (their negative aspects) e strengths (your positive differentiators). | Improve competitive positioning by targeting messaging at competitor weaknesses, increase win rate by 15-25% through data-driven differentiation, guide product strategy with competitive gap analysis. |
...
**Text Preprocessing:**
- Clean HTML tags e special characters before submission
...
**Batch Optimization:**
- Optimal batch size: 50-100 texts per request (balances latency e throughput)
...
- Useful for bre monitoring e competitive intelligence
...
- All text data encrypted in transit (TLS 1.3) e em repouso
...
- Idioms e cultural expressions may have lower accuracy
...
- [NPS](../CustomerIntelligence/NPS.md) - Net Promoter Score analysis e predictions
- [Customer Loyalty](../CustomerIntelligence/CustomerLoyalty.md) - Measure e predict customer loyalty
