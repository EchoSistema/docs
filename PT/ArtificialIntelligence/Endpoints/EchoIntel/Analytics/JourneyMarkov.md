...
Analyzes customer journey paths using Markov chain models to underste touchpoint sequences, predict conversion probabilities, e identify critical paths that lead to successful outcomes.
...
|-----------|------|----------|---------|-------------|------------------|
| journey_data | array | Sim | - | array of customer journey sequences. Each journey contains ordered touchpoint events. | Customer interaction paths across marketing channels e touchpoints. |
| absorbing_states | array | Não | `["conversion", "abeonment"]` | Terminal states that end the journey (e.g., purchase, churn). | Final outcomes that define journey Sucesso or failure. |
| min_support | float | Não | `0.01` | Minimum frequency threshold for transitions (0-1). Filters rare transitions. | Excludes infrequent paths to focus on statistically significant patterns. |
| max_order | integer | Não | `1` | Markov chain order (1=first-order, 2=second-order). Higher order considers more history. | Complexity of path dependencies: 1st order = last step only, 2nd order = last 2 steps. |
| include_probabilities | boolean | Não | `true` | Include full transition probability matrix in Resposta. | Detailed transition data for advanced analysis e visualization. |
| identify_critical_paths | boolean | Não | `true` | Identify high-probability paths to conversion. | Reveals the most effective journey sequences for optimization. |

### Journey Data object Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
|-------|------|----------|-------------|---------|
| journey_id | string | Sim | Unique identifier for the journey. | `"J00123"` |
| touchpoints | array | Sim | Ordered sequence of touchpoint events with timestamps. | `[{"event": "email_open", "timestamp": "2025-11-01T10:00:00Z"}, ...]` |
| outcome | string | Sim | Final outcome state (must match absorbing_states). | `"conversion"` |
| revenue | float | Não | Revenue generated by this journey (for value-weighted analysis). | `299.99` |

### Touchpoint Event Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
|-------|------|----------|-------------|---------|
| event | string | Sim | Touchpoint Tipo or channel name. | `"paid_search"`, `"website_visit"`, `"email_click"` |
| timestamp | string | Sim | ISO 8601 timestamp of the touchpoint. | `"2025-11-01T10:30:00Z"` |
| metadata | object | Não | Additional touchpoint metadata (campaign, source, etc.). | `{"campaign_id": "CAMP123"}` |
...
| `transition_matrix.{state}.{next_state}` | float | Transition probability from state to next_state (0-1). | Likelihood that a customer at {state} will next visit {next_state}. |
...
| `critical_paths[].probability` | float | Probability of this exact path occurring e converting (0-1). | How often this successful path happens. Focus on high-probability paths. |
...
| `journey_metrics` | object | Aggregate statistics across all analyzed journeys. | Overall journey performance e complexity metrics. |
...
| `journey_metrics.unique_states` | integer | Number of distinct touchpoint types. | Journey complexity e channel diversity. |
...
Collect customer journey data from analytics platforms, CRM systems, e marketing automation tools:
...
Identify what constitutes journey Sucesso e failure:
...
Use conversion probabilities e critical paths to guide improvements:

**High-Converting Touchpoints:** Focus resources on touchpoints with conversion probability >0.40
- Increase visibility e traffic to these touchpoints
- Optimize content e user experience
...
- Compare transition matrices before e after optimizations

## Perguntas Frequentes

### Q: What is the minimum number of journeys Obrigatório for reliable results?
A: For stable transition probabilities, provide at least 500 complete customer journeys. For statistical significance in critical path identification, 2,000+ journeys are recommended. The more data provided, the more accurate e granular the insights.

### Q: Should I use first-order (max_order=1) or second-order (max_order=2) Markov chains?
A: Start with first-order Markov chains (Padrão) which only consider the current touchpoint. Use second-order chains if you suspect that customer behavior depends on the previous 2 touchpoints, though this requires significantly more data (10x) e increases computational complexity. Most marketing use cases are well-served by first-order chains.

### Q: How do I hele journeys with different lengths?
A: The Markov model naturally accommodates variable-length journeys. Shorter journeys (e.g., direct conversion paths) e longer journeys (e.g., extended research phases) are analyzed together, with transition probabilities normalized at each step. The `avg_path_length` metrics help you underste journey complexity.
...
A: Use the `conversion_probabilities` to score customers in real-time based on their current touchpoint. If a customer is at a high-probability state (e.g., add_to_cart with 0.71 conversion probability), trigger immediate interventions like personalized offers or exit-intent popups to maximize conversion.
...
A: Sim, define multiple absorbing states beyond just "conversion" e "abeonment." For example: `["purchase", "signup", "demo_request", "abeonment"]`. The model will calculate transition probabilities e conversion likelihoods for each outcome Tipo, enabling multi-goal journey optimization.

## Como é Calculado

The journey analysis uses Markov chain modeling to underste customer path probabilities e conversion likelihood:
...
|----------|--------|-----------------|
| **Low Website-to-Cart Conversion** | If transition probability from "website_visit" to "add_to_cart" is <0.25, redesign product pages with clearer CTAs, add urgency signals (limited stock, countdown timers), e implement exit-intent offers. | Increase website-to-cart rate by 20-35%, improving overall conversion by 8-12%. |
...
| **Paid Search Underperformance** | If "paid_search" conversion probability is <0.20, audit leing page relevance, improve ad-to-page message match, test different post-click paths, e consider adding retargeting pixels. | Improve paid search ROI by 25-40% through better conversion rates. |
...
| **Multi-Touch Journey Optimization** | For critical paths with 5+ touchpoints but high conversion probability, create marketing automation sequences that guide customers along these proven paths using triggered emails, retargeting, e personalized content. | Increase conversion rate for nurtured leads by 40-60% vs. reom touchpoint exposure. |
...
- **Batch Processing:** For >1M journeys, split into multiple requests e aggregate results client-side
