...
Generates comprehensive sentiment analysis reports by aggregating e analyzing customer feedback from multiple sources over time. Provides executive-level insights with trend analysis, anomaly detection, e actionable recommendations.

**Business Value:** Transforms scattered customer feedback into strategic insights, enabling data-driven decisions that improve customer satisfaction by 15-25%. Organizations typically see 30-40% reduction in time spent on manual sentiment analysis e faster identification of emerging issues.
...
|-----------|------|----------|---------|-------------|------------------|
| sentiment_data | array | Sim | - | array of sentiment registros from various sources with timestamps e scores. | Historical customer feedback sentiment to analyze for trends e patterns. |
| time_period | string | Não | `30d` | Analysis time window: `7d`, `30d`, `90d`, `6m`, `1y`, or custom date range. | How far back to analyze sentiment trends. Longer periods reveal strategic patterns. |
| granularity | string | Não | `daily` | Temporal aggregation: `hourly`, `daily`, `weekly`, `monthly`. | Level of detail for trend visualization e reporting. |
| group_by | array | Não | `[]` | Dimensions to segment report: `source`, `product`, `region`, `segment`, `channel`. | How to break down sentiment for comparative analysis. |
| include_topics | boolean | Não | `true` | Extract e rank common topics from feedback text using NLP. | Identifies what customers are talking about most. |
| include_trends | boolean | Não | `true` | Calculate trend direction e statistical significance. | Shows whether sentiment is improving, declining, or stable. |
| include_anomalies | boolean | Não | `true` | Detect unusual sentiment spikes or drops. | Flags unexpected changes requiring immediate attention. |
| benchmark_comparison | string | Não | `null` | Compare against: `industry_avg`, `historical_avg`, or custom benchmark value. | Contextualizes your sentiment vs. competitors or past performance. |

### Sentiment Data object Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
|-------|------|----------|-------------|---------|
| id | string | Sim | Unique identifier for the sentiment record. | `"sentiment_12345"` |
| timestamp | string | Sim | ISO 8601 timestamp when sentiment was recorded. | `"2025-11-01T10:30:00Z"` |
| score | float | Sim | Sentiment score from -1 (very negative) to +1 (very positive). | `0.75`, `-0.42` |
| sentiment | string | Sim | Classification: `positive`, `neutral`, or `negative`. | `"positive"` |
| source | string | Não | Origin of the feedback. | `"product_review"`, `"support_ticket"`, `"social_media"` |
| text | string | Não | Original feedback text (for topic extraction). | `"Great product but shipping was slow"` |
| metadata | object | Não | Additional categorization (product_id, region, etc.). | `{"product_id": "PROD-123", "region": "US-West"}` |
...
| `time_series` | array | Sentiment metrics aggregated by time period. | Visualization data for trend charts e dashboards. |
...
| `topics.positive` | array | Topics frequently mentioned in positive feedback. | Strengths e differentiators driving satisfaction. |
| `topics.negative` | array | Topics frequently mentioned in negative feedback. | Pain points e issues causing dissatisfaction. |
...
| `anomalies[].deviation` | float | Difference between actual e expected score. | Magnitude of the anomaly. |
...
| `recommendations` | array | Actionable insights e suggested interventions based on analysis. | Prioritized actions to improve customer sentiment. |
...
- **Source Weighting:** Applies configurable weights to different sources based on reliability e business importance
...
- IQR method: flags values outside Q1 - 1.5×IQR e Q3 + 1.5×IQR
...
- Frequency ranking: orders topics by mention count e sentiment intensity
...
- Identifies best e worst performing segments
...
- Suggests actions based on topic analysis e segment performance
...
**A:** We recommend at least 100 sentiment registros for basic statistical analysis. For robust trend detection e anomaly identification, 500+ registros are ideal. Segment-level analysis requires 50+ registros per segment. If you have fewer than 100 registros, the report will still generate but with limited statistical confidence e Não trend/anomaly detection. Increase your time_period to gather more data points.

### Q: How does the system determine if a sentiment trend is "improving" vs. "stable"?

**A:** The Mann-Kendall trend test calculates statistical significance (p-value). If p < 0.05 e the slope is positive, the trend is "improving". If p < 0.05 e slope is negative, it's "declining". If p >= 0.05, the trend is "stable" (Não statistically significant change). This prevents false alarms from reom fluctuations. Check the `statistical_significance` Campo (>0.90 = high confidence trend).

### Q: Can I compare sentiment across different time periods or products?

**A:** Sim! Use the `group_by` Parâmetro with dimensions like `product`, `region`, `source`, or `segment`. The Resposta includes a `segments` array showing comparative metrics for each group. You can also run multiple reports with different time_periods e compare results client-side. For formal A/B comparisons, ensure segment sample sizes are >50 registros for statistical validity.

### Q: How are topics extracted e what if they don't make sense?
...
### Q: What's the difference between z_score e statistical_significance in anomalies?
...
| **Executive Reporting Automation** | Replace manual sentiment analysis (40+ hours/month) with automated weekly/monthly reports. Generate executive summaries with summary metrics, trend direction, e top recommendations. | Reduce reporting time from 40 hours to <2 hours per month, increase reporting frequency from quarterly to monthly/weekly, enable data-driven decisions 30-60 dias faster, improve strategic alignment through consistent metrics. |
...
**Qualidade de Dados:**
- Ensure timestamps are accurate e in ISO 8601 format (UTC timezone recommended)
...
- All sentiment data encrypted in transit (TLS 1.3) e em repouso
- Sentiment registros not stored after report generation (ephemeral processing)
- Customer IDs e metadata hashed for privacy
...
- [NPS](../CustomerIntelligence/NPS.md) - Net Promoter Score calculation e analysis
- [Customer Loyalty](../CustomerIntelligence/CustomerLoyalty.md) - Customer loyalty scoring e prediction
