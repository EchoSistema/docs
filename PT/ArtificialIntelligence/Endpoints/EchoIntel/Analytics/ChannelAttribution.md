...
Calculates marketing channel contribution to conversions using multiple attribution models (first-touch, last-touch, linear, time-decay, position-based, e data-driven Shapley value) to determine accurate channel ROI e optimize marketing spend allocation.
...
|-----------|------|----------|---------|-------------|------------------|
| journey_data | array | Sim | - | array of customer journey objects containing touchpoint sequences e conversion outcomes. | Complete customer interaction history across all marketing channels. |
| attribution_models | array | Não | `["last_touch", "first_touch", "linear", "time_decay", "shapley"]` | Attribution models to apply. Available: `first_touch`, `last_touch`, `linear`, `time_decay`, `position_based`, `shapley`. | Different methodologies for crediting channels. Use multiple models to compare e validate insights. |
| time_decay_half_life | integer | Não | `7` | Half-life in dias for time-decay model (how quickly touchpoint credit decreases). | Controls how much recent touchpoints are favored. 7 dias = steard, 1 day = aggressive recency bias. |
| position_based_weights | object | Não | `{"first": 0.4, "last": 0.4, "middle": 0.2}` | Weight distribution for position-based attribution (first, last, middle touchpoints). | Customize credit allocation: 40% first touch, 40% last touch, 20% distributed to middle. |
| conversion_value_field | string | Não | `"revenue"` | Campo name containing conversion value (for revenue-weighted attribution). | Which metric to use for attribution: revenue, profit, lifetime_value, etc. |
| include_comparison | boolean | Não | `true` | Include side-by-side comparison of all attribution models. | Enables model comparison table to see how different models credit each channel. |
| group_by_period | string | Não | `null` | Group attribution results by time period: `daily`, `weekly`, `monthly`. | Track channel attribution trends over time. |

### Journey Data object Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
|-------|------|----------|-------------|---------|
| journey_id | string | Sim | Unique identifier for the customer journey. | `"J12345"` |
| touchpoints | array | Sim | Ordered array of marketing touchpoint interactions. | `[{"channel": "paid_search", "timestamp": "2025-11-01T10:00:00Z"}, ...]` |
| converted | boolean | Sim | Whether the journey resulted in conversion. | `true` |
| conversion_value | float | Condicional | Value of the conversion (Obrigatório if using value-weighted attribution). | `299.99` |
| conversion_timestamp | string | Não | ISO 8601 timestamp of conversion event. | `"2025-11-05T14:30:00Z"` |

### Touchpoint object Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
|-------|------|----------|-------------|---------|
| channel | string | Sim | Marketing channel name (e.g., paid_search, email, organic, social). | `"paid_search"` |
| timestamp | string | Sim | ISO 8601 timestamp of the touchpoint interaction. | `"2025-11-01T10:00:00Z"` |
| campaign | string | Não | Campaign identifier for detailed attribution. | `"SUMMER2025"` |
| cost | float | Não | Cost associated with this touchpoint (for ROI calculation). | `2.50` |
...
| `model_comparison.comparison_matrix` | array | Matrix where rows are channels e columns are models, values are percentage attribution. | Quick visual comparison of model differences. Identify channels with consistent vs. variable attribution. |
...
| `summary` | object | Aggregate statistics across all journeys e models. | Overall campaign performance e data quality indicators. |
...
| `summary.total_journeys_analyzed` | integer | Number of customer journeys analyzed (converted e non-converted). | Sample size for statistical confidence. |
...
### 1. Data Collection
Extract customer journey e touchpoint data from marketing platforms:
...
- Balance short-term (last-touch) e long-term (first-touch) value

### 7. Monitor e Iterate
Track attribution over time:
- Re-run monthly to detect shifts in channel effectiveness
- Compare attribution before e after marketing changes
- Use `group_by_period` to analyze seasonal trends

## Perguntas Frequentes

### Q: Which attribution model should I use?
A: Start with Shapley value for multi-touch journeys as it provides the most statistically rigorous attribution based on game theory. Use first-touch to underste awareness drivers, last-touch for immediate conversion drivers, e time-decay for recency-weighted attribution. Running multiple models simultaneously helps validate insights.
...
A: Industry benchmarks vary widely. E-commerce typically aims for 400-600% ROAS (4-6x return), B2B SaaS for 300-500%, e bre awareness for 200-300%. Use your own historical data as a baseline e optimize from there. ROAS below 100% means you're spending more than you're earning.

### Q: Should I only look at last-touch attribution?
A: Não. Last-touch significantly undervalues awareness e nurturing channels. A customer might discover your bre through paid social, research via organic search, e convert via email. Last-touch gives 100% credit to email, ignoring the critical role of paid social e organic. Multi-touch attribution reveals the full customer journey.
...
| **Email Undervalued** | If email shows <10% last-touch attribution but 25%+ in linear or Shapley, it's a critical nurturing channel. Increase email cadence e personalization. | Improve conversion rates by 20-30% through better email nurturing. |
| **Paid Social Low ROI** | If paid social ROAS <200% e Shapley attribution <15%, reduce spend or shift to retargeting only. Test creative e audience targeting before cutting entirely. | Reduce wasted ad spend by 30-40%, reallocate to higher-ROI channels. |
...
| **First-Touch Dominance** | If first-touch e last-touch agree on top channels, your funnel is efficient. Optimize conversion rates on these proven channels rather than expeing channel mix. | Increase conversion rate by 15-25% through channel-specific optimization. |
| **Attribution Volatility** | If model_agreement_score <0.50, your journeys are highly variable. Segment customers by journey Tipo e create channel strategies for each segment. | Improve marketing efficiency by 20-30% through segment-specific tactics. |
...
- **Step 1:** Aggregate customer journey paths from all conversion e non-conversion events
- **Step 2:** For each model, calculate attribution weights based on touchpoint positions e timestamps
...
- **Time-Decay** provides a good balance between accuracy e performance for most use cases
- **First/Last-Touch** are useful for baseline comparisons e simple attribution scenarios
...
- **Use when:** First e last touches are most valuable (U-shaped funnel)
- **Best for:** Traditional marketing funnels with clear awareness e conversion stages
...
- Remove bot traffic e invalid interactions

**Journey Completeness:**
- Include all touchpoints, not just paid channels
- Track organic, direct, e referral traffic
- Capture both online e offline touchpoints when possible
- Ensure conversion timestamp is after last touchpoint

### Desempenho e Scalability
...
**Attribution Window:**
- Padrão: 30-day attribution window (touchpoints within 30 dias of conversion)
...
- All journey data is encrypted in transit (TLS 1.3) e em repouso
- Journey IDs are hashed internally for privacy
- Cost data is aggregated e anonymized in reports
...
- [Sentiment Analysis](SentimentAnalysis.md) - Analyze sentiment of customer feedback e reviews
- [Customer Segmentation](../CustomerIntelligence/CustomerSegmentation.md) - Segment customers based on behavior e characteristics
