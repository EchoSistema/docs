...
Extracts engineered features from customer data for Customer Lifetime Value (CLV) prediction modeling. This Endpoint prepares behavioral, transactional, e demographic features that serve as inputs for CLV forecasting algorithms.
...
| --------- | ---- | -------- | ----------- | ---------------- | ------- |
| customer_data | array | Sim | array of customer registros with transaction history. | Customer profiles with purchase behavior data. | - |
| feature_set | string | Não | Feature group to generate: `basic`, `advanced`, `all`. | Determines complexity of feature engineering. | `all` |
| time_window | integer | Não | Rolling window in dias for temporal features (30-365). | Analysis period for trend calculation. | `90` |
| include_metadata | boolean | Não | Include feature engineering metadata e descriptions. | Adds explanations for each computed feature. | `false` |

### Customer Data object Fields

| Campo | Tipo | Obrigatório | Descrição | Significado Empresarial | Example |
| ----- | ---- | -------- | ----------- | ---------------- | ------- |
| customer_id | string | Sim | Unique customer identifier. | Customer account ID or email. | `"CUST-10234"` |
| transactions | array | Sim | array of transaction registros. | Purchase history with dates e amounts. | See below |
| join_date | string | Não | Customer registration date (ISO 8601). | Account creation date for tenure calculation. | `"2023-01-15"` |
| demographics | object | Não | Demographic attributes (age, location, segment). | Customer profile information. | `{"age": 34, "country": "US"}` |

### Transaction object Fields

| Campo | Tipo | Obrigatório | Descrição | Example |
| ----- | ---- | -------- | ----------- | ------- |
| transaction_id | string | Sim | Unique transaction identifier. | `"TXN-789012"` |
| date | string | Sim | Transaction date (ISO 8601). | `"2025-10-15"` |
| amount | float | Sim | Transaction amount in currency units. | `149.99` |
| product_category | string | Não | Product category or SKU. | `"Electronics"` |
| channel | string | Não | Purchase channel: `online`, `mobile`, `store`. | `"online"` |
...
| `features[].temporal_features` | object | Time-based behavior patterns. | Trends e velocity metrics. |
...
| `metadata` | object | Feature engineering metadata. | Processing information e versioning. |
...
| `metadata.computation_time_ms` | integer | Duração do processamento em milissegundos. | Métrica de desempenho. |
...
The system computes statistical e behavioral features that correlate with customer lifetime value:
...
1. **Data Aggregation:** Collect customer transactional, behavioral, e demographic data
...
- **Tamanho do Lote:** Process up to 50,000 customers per request
...
- **Qualidade de Dados:** Ensure transaction dates are accurate e amounts are positive
...
3. Use predictions for customer segmentation e targeting
...
**A:** Demographic fields (age, location) are optional e do not affect core feature extraction. If provided, they're passed through in the Resposta for downstream model usage. Missing demographics don't impact transactional, temporal, or behavioral feature computation.

### Q: Can I use these features directly for CLV prediction?
**A:** Sim! These features are designed as inputs for CLV forecasting models. Use the `all` feature set for comprehensive modeling. The features are pre-normalized e ready for machine learning algorithms. For automated CLV prediction, use the [CLV Forecast](CustomerClvForecast.md) Endpoint, which internally uses these features.

### Q: What does the trend_coefficient represent e how is it calculated?
...
**A:** For real-time applications (e.g., personalization engines), refresh features weekly or when significant customer actions occur (new purchase, large order). For monthly reporting e batch campaigns, refresh mensal. Features are stateless—each API call computes fresh features from provided transaction data without caching.
...
2. Identify customers with low diversity (<3 categories) e high frequency (5+ purchases)
...
4. Monitor conversion rates e AOV by segment
