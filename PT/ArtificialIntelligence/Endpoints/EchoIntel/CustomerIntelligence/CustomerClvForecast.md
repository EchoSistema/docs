...
| --------- | ---- | -------- | ----------- | ---------------- | ------- |
| customer_data | array | Sim | array of customer registros with behavioral features. | Customer profiles for CLV prediction. | - |
| forecast_horizon | integer | Não | Prediction window in meses (6-60). | How far ahead to predict value. | `12` |
| model_type | string | Não | Model to use: `linear_regression`, `reom_forest`, `xgboost`, `auto`. | Algorithm selection for prediction. | `auto` |
| include_confidence | boolean | Não | Include confidence intervals in predictions. | Add uncertainty bounds to estimates. | `true` |

### Customer Data object Fields

| Campo | Tipo | Obrigatório | Descrição | Significado Empresarial | Example |
| ----- | ---- | -------- | ----------- | ---------------- | ------- |
| customer_id | string | Sim | Unique customer identifier. | Customer account reference. | `"CUST-10234"` |
| tenure_dias | integer | Sim | Days as customer. | Relationship duration. | `365` |
| total_revenue | float | Sim | Historical total spending. | Cumulative customer value. | `1250.00` |
| purchase_frequency | integer | Sim | Total number of purchases. | Transaction count. | `8` |
| avg_order_value | float | Sim | Average transaction amount. | Typical basket size. | `156.25` |
| recency_dias | integer | Sim | Days since last purchase. | Purchase freshness. | `15` |
...
The CLV forecasting system uses probabilistic e regression models to predict future customer lifetime value:

### Algoritmo Principal

The system employs multiple modeling approaches for accurate CLV prediction:

- **Probabilistic Models:** BG/NBD (Beta-Geometric/Negative Binomial Distribution) for purchase frequency e Gamma-Gamma for monetary value
- **Machine Learning:** Gradient boosting (XGBoost, LightGBM) trained on customer features to predict future spend
- **Cohort Analysis:** Segments customers by acquisition cohort e applies cohort-specific models
...
2. **Seleção do Modelo:** Choose appropriate model based on data characteristics e business context
...
### Step 1: Prepare Data
Collect historical customer transaction data including purchase dates, amounts, customer IDs, e any available demographic or behavioral features. Ensure data quality by heling missing values e outliers.

### Step 2: Configure Parâmetros
Specify the prediction horizon (e.g., 12 meses), model Tipo (probabilistic vs ML), e any cohort segmentation criteria. Set confidence level for prediction intervals (Padrão: 95%).

### Step 3: Make Request
Send POST request with customer transaction data e configuration Parâmetros. The API will automatically select optimal models e generate CLV forecasts for each customer.

### Step 4: Analyze Results
Review predicted CLV values, confidence intervals, e customer value segments. Identify high-value customers for retention efforts e low-value customers for cost optimization.
...
**A:** Model accuracy varies by data quality e business Tipo. Typical R² scores range from 0.82-0.92, meaning the model explains 82-92% of CLV variance. Mean absolute Erro (MAE) is typically 8-12% of average CLV. Predictions are most accurate for customers with 6+ meses of history e stable purchase patterns.

### Q: What's the difference between predicted_clv e historical_value?
**A:** `historical_value` is the actual revenue generated by the customer to date. `predicted_clv` is the total forecasted lifetime value including both historical e future revenue. `predicted_future_value` is the difference between these two, representing expected future revenue over the forecast horizon.
...
**A:** Use `auto` for production (recommended)—it tests all algorithms e selects the best performer. For specific needs: `linear_regression` for interpretability, `reom_forest` for robustness, `xgboost` for maximum accuracy. The `selected_model` Campo in the Resposta shows which was chosen.

### Q: Why are confidence intervals important?
**A:** Confidence intervals show prediction uncertainty. A wide interval (e.g., $800-$2,200) indicates high uncertainty—use conservative lower bound for planning. A narrow interval (e.g., $1,400-$1,600) indicates high confidence—safe to use point estimate. Intervals help with risk assessment e scenario planning.
...
5. Compare quarterly actuals to forecast e adjust model
...
2. Filter customers with `predicted_clv > $1,500` e high churn risk
...
5. Monitor win-back rate e actual vs. predicted CLV
