...
Automatically generates churn labels for customer datasets by applying rule-based classification algorithms that identify churned customers based on behavioral patterns, subscription status, e engagement metrics.
...
|-----------|------|----------|---------|-------------|------------------|
| customer_data | array | Sim | - | array of customer activity registros with timestamps e behavioral metrics. | Historical customer interaction data to analyze for churn patterns. |
| churn_definition | object | Não | Auto | Customizable rules defining what constitutes churn for your business model. | Business-specific criteria: inactivity period, cancellation events, engagement thresholds. |
| lookback_period | integer | Não | 180 | Days to look back for customer activity history. | How far back to analyze behavior (e.g., 180 dias = 6 meses). |
| inactivity_threshold | integer | Não | 90 | Days of inactivity after which customer is labeled as churned. | Time without engagement that indicates abeonment (e.g., 90 dias for SaaS). |
| include_reactivations | boolean | Não | false | Track customers who churned then returned (useful for win-back analysis). | Identifies customers who left but came back, for retention pattern analysis. |
| min_tenure | integer | Não | 30 | Minimum dias as customer before churn labeling applies (filters early dropouts). | Excludes very new customers who never properly onboarded. |
...
| signup_date | string | Sim | ISO 8601 date when customer signed up. | `"2024-01-15"` |
| last_activity | string | Sim | ISO 8601 date of most recent customer activity. | `"2024-11-01"` |
| subscription_status | string | Não | Current subscription state: `active`, `cancelled`, `suspended`, `expired`. | `"active"` |
| total_transactions | integer | Não | Total number of transactions/interactions. | `45` |
| lifetime_value | float | Não | Total revenue from customer. | `1250.00` |
| cancellation_date | string | Não | ISO 8601 date when customer explicitly canceled (if applicable). | `"2024-10-15"` |
...
| `labeled_customers[].dias_since_last_activity` | integer | Days between last activity e analysis date. | Recency metric: higher values indicate disengagement. |
...
| `labeled_customers[].label_reason` | string | Explanation for why churn label was assigned. | Transparency for validation e rule tuning. |
| `summary` | object | Aggregate statistics across all labeled customers. | Overall churn rate e dataset health metrics. |
...
| `data_quality` | object | Data quality metrics e validation results. | Identifies missing data or invalid registros. |
...
1. **Data Collection:** Aggregate customer transaction, engagement, e subscription data
2. **Rule Application:** Apply configurable business rules to identify churn events e dates
3. **Label Assignment:** Create binary labels (churned=1, active=0) with churn timestamps
4. **Validation:** Verify label consistency e hele edge cases (reactivations, seasonality)
...
- **Tempo de Processamento:** 100-300ms for 10,000 customers
- **Requisitos de Dados:** Minimum 6 meses of customer activity history
...
Monitor labeling quality e adjust rules:
...
### Q: How do I hele customers who churn e then return (reactivations)?
...
- **Taxa de Transferência:** 10,000 customers processed in 100-300ms
...
- All customer data encrypted in transit (TLS 1.3) e em repouso
