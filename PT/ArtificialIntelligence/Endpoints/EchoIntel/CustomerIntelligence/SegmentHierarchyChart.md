...
|-----------|------|----------|---------|-------------|------------------|
| customer_data | array | Sim | - | array of customer registros with features for hierarchical clustering. | Customer attributes used to build segment hierarchy. |
| linkage_method | string | Não | `"ward"` | Clustering linkage: `ward`, `complete`, `average`, `single`. | How segments merge. Ward minimizes within-cluster variance (recommended). |
| distance_metric | string | Não | `"euclidean"` | Distance calculation: `euclidean`, `manhattan`, `cosine`. | How customer similarity is measured. |
| max_levels | integer | Não | `5` | Maximum hierarchy depth (number of nested segment levels). | How granular the segmentation goes. 3-5 levels typical. |
| min_segment_size | integer | Não | `100` | Minimum customers per segment at any level. | Prevents overly small, non-actionable segments. |
| include_dendrogram_data | boolean | Não | `true` | Return dendrogram structure for visualization. | Enables chart rendering in BI tools. |
...
| features | object | Sim | Customer attributes for clustering (RFM, demographics, behavior). | `{"recency": 30, "frequency": 5, "monetary": 500}` |
...
| `hierarchy.level_{n}[].segment_id` | string | Unique segment identifier. | Reference for targeting e reporting. |
...
Submit to API with appropriate max_levels (3-5 typical) e min_segment_size (50-200).

### 3. Analyze Hierarchy Levels
- **Level 1-2:** Broad segments for strategic planning e bre campaigns
...
Choose hierarchy level based on campaign Tipo e resources.
...
### Q: What's the difference between this e regular segmentation?
...
3. **Dendrogram Construction:** Build hierarchical tree structure recording merge sequence e distances
4. **Level Selection:** Identify meaningful cut-off points (using dendrogram gaps or business criteria)
5. **Visualization:** Generate chart with segment hierarchy, sizes, e nesting relationships
