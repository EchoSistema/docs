...
|-------|------|----------|-------------|---------|
| journey_id | string | Sí | Unique identifier for the journey. | `"J00123"` |
| touchpoints | array | Sí | Ordered sequence of touchpoint events with timestamps. | `[{"event": "email_open", "timestamp": "2025-11-01T10:00:00Z"}, ...]` |
| outcome | string | Sí | Final outcome state (must match absorbing_states). | `"conversion"` |
...
| Campo | Tipo | Requerido | Descripción | Example |
|-------|------|----------|-------------|---------|
| event | string | Sí | Touchpoint Tipo or channel name. | `"paid_search"`, `"website_visit"`, `"email_click"` |
| timestamp | string | Sí | ISO 8601 timestamp of the touchpoint. | `"2025-11-01T10:30:00Z"` |
...
| `critical_paths[].probability` | float | Probability of this exact path occurring y converting (0-1). | How often this successful path happens. Focus on high-probability paths. |
...
Collect customer journey data from analytics platforms, CRM systems, y marketing automation tools:
...
- Increase visibility y traffic to these touchpoints
- Optimize content y user experience
