...
Discovers frequent sequential patterns in customer journeys using pattern mining algorithms to identify common touchpoint sequences, revealing how customers typically navigate across channels before converting or abyoning.
...
| group_by_outcome | boolean | No | `true` | Separate patterns by outcome (conversion vs. abyonment). | Distinguish successful vs. failed journey patterns. |
...
| outcome | string | Sí | Journey outcome: `conversion` or `abyonment`. | `"conversion"` |
...
| `pattern_divergence` | array | Points where conversion y abyonment paths diverge. | Critical decision points in the journey. |
...
- **Abyonment patterns:** Identify drop-off points y fix friction
...
A: Increase min_support or reduce max_pattern_length. Focus on patterns with high support (>0.20) y patterns that diverge between converters y abyoners.

## Notas

### Rendimiento
- **Tiempo de Procesamiento:** 300-600ms for 100,000 journeys
- **Requisitos de Datos:** Minimum 1,000 journeys for meaningful patterns
...
- **Friction detection:** Find abyonment patterns to fix
...
- **Step 2:** Mine frequent sequences using minimum support (Por Defecto: 1% of all journeys)
...
- **Tiempo de Procesamiento:** 300-600ms for 100,000 journey sequences
- **Requisitos de Datos:** Minimum 1,000 journeys for meaningful pattern discovery
...
- [Propensity to Respond to Campaign](../Propensity/PropensityRespondCampaign.md) - Predict campaign Respuesta likelihood
