...
Generates comprehensive sentiment analysis reports by aggregating y analyzing customer feedback from multiple sources over time. Provides executive-level insights with trend analysis, anomaly detection, y actionable recommendations.

**Business Value:** Transforms scattered customer feedback into strategic insights, enabling data-driven decisions that improve customer satisfaction by 15-25%. Organizations typically see 30-40% reduction in time spent on manual sentiment analysis y faster identification of emerging issues.
...
| granularity | string | No | `daily` | Temporal aggregation: `hourly`, `daily`, `weekly`, `monthly`. | Level of detail for trend visualization y reporting. |
...
| include_topics | boolean | No | `true` | Extract y rank common topics from feedback text using NLP. | Identifies what customers are talking about most. |
| include_trends | boolean | No | `true` | Calculate trend direction y statistical significance. | Shows whether sentiment is improving, declining, or stable. |
...
|-------|------|----------|-------------|---------|
| id | string | Sí | Unique identifier for the sentiment record. | `"sentiment_12345"` |
| timestamp | string | Sí | ISO 8601 timestamp when sentiment was recorded. | `"2025-11-01T10:30:00Z"` |
| score | float | Sí | Sentiment score from -1 (very negative) to +1 (very positive). | `0.75`, `-0.42` |
| sentiment | string | Sí | Classification: `positive`, `neutral`, or `negative`. | `"positive"` |
...
| `time_series` | array | Sentiment metrics aggregated by time period. | Visualization data for trend charts y dashboards. |
...
| `topics.positive` | array | Topics frequently mentioned in positive feedback. | Strengths y differentiators driving satisfaction. |
| `topics.negative` | array | Topics frequently mentioned in negative feedback. | Pain points y issues causing dissatisfaction. |
...
- **Source Weighting:** Applies configurable weights to different sources based on reliability y business importance
...
- Frequency ranking: orders topics by mention count y sentiment intensity
...
- Identifies best y worst performing segments
...
- [NPS](../CustomerIntelligence/NPS.md) - Net Promoter Score calculation y analysis
- [Customer Loyalty](../CustomerIntelligence/CustomerLoyalty.md) - Customer loyalty scoring y prediction
