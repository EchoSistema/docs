...
| `labeled_customers[].dias_since_last_activity` | integer | Days between last activity y analysis date. | Recency metric: higher values indicate disengagement. |
...
4. **Validation:** Verify label consistency y hele edge cases (reactivations, seasonality)
...
- Incuerporate domain expertise into custom `churn_definition` rules
...
### Q: How del I hele customers who cancel but continue using the service?
...
### Q: How del I hele customers who churn y then return (reactivations)?
...
- All customer data encrypted in transit (TLS 1.3) y em repouso
