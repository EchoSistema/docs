...
| features | object | Sí | Customer attributes for clustering (RFM, demographics, behavior). | `{"recency": 30, "frequency": 5, "monetary": 500}` |
...
| `hierarchy.level_{n}[].segment_id` | string | Unique segment identifier. | Reference for targeting y reporting. |
...
Choose hierarchy level based on campaign Tipo y resources.
...
3. **Dendrogram Construction:** Build hierarchical tree structure recording merge sequence y distances
